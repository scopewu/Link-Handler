...
- **纯锚点链接不再被改写**
  `href` 为空或为 `#...` 的链接会被浏览器按当前页面地址解析，页面自身的查询
  参数会被误当作链接参数。重定向解包与跟踪清理现在跳过纯锚点链接，避免同页
  锚点被改写成带页面参数的绝对地址（锚点滚动变成整页跳转）。

- **选项页防抖保存竞态**
  防抖保存捕获的是调用时刻的配置对象，「恢复默认」或「导入」之后若仍有挂起的
  保存，旧引用会把新配置覆盖回去。定时器现在触发时读取实时配置，且重置/导入
  会取消挂起的保存。

- **popup 白名单开关回滚**
  白名单保存失败时，popup 的开关与图标会恢复到存储中的状态并显示错误提示，
  不再停留在从未持久化的状态。

### 内部改进

- **自托管字体**
  扩展页面改为内置 Outfit / Plus Jakarta Sans（拉丁可变字重 woff2），通过本地
  `fonts.css` 加载，取代远程 Google Fonts 请求——选项页与 popup 完全离线可用。

- i18n：跟踪规则参数标签改用独立的 `cleanUrlParamsShort` 消息，不再拆分语言相关
  字符串；移除无用的 `format()` 辅助函数与未使用的 `saveSettings` 键。
- `parseInt` 补充进制参数；非扩展环境下对 `chrome.tabs` 访问增加防护。
- `prefers-reduced-motion` 样式去重合并到共享的 `base.css`；精简 README 重复内容。
...
- **内置标识**
  内置的重定向/跟踪规则和内置白名单域名在设置页显示「内置」徽章，
  与用户自行添加的条目一目了然。渲染时与 `DEFAULT_CONFIG` 比对判定，
  用户修改过的内置规则仍可正确识别。已适配 en / zh_CN / zh_TW 三种语言。
